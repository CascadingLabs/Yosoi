...
async def test_record_url_increments_url_count(tracker):
    ...
    assert await tracker.get_url_count('example.com') == 1
...
async def test_record_url_with_llm_increments_llm_calls(tracker):
    ...
    assert await tracker.get_llm_calls('example.com') == 1


async def test_record_url_without_llm_does_not_increment_llm_calls(tracker):
    await tracker.record_url('https://example.com/article', used_llm=False)
    assert await tracker.get_llm_calls('example.com') == 0
...
async def test_multiple_records_accumulate(tracker):
    ...
    assert await tracker.get_url_count('example.com') == 3
    assert await tracker.get_llm_calls('example.com') == 2
...
async def test_multiple_domains_tracked_independently(tracker):
    ...
    assert await tracker.get_url_count('example.com') == 1
    assert await tracker.get_url_count('other.com') == 1
    assert await tracker.get_llm_calls('example.com') == 1
    assert await tracker.get_llm_calls('other.com') == 1


async def test_get_stats_returns_domain_stats(tracker):
    await tracker.record_url('https://example.com/x', used_llm=True)
    stats = await tracker.get_stats('example.com')
    assert hasattr(stats, 'llm_calls')
    assert hasattr(stats, 'url_count')


async def test_get_stats_unknown_domain_returns_zeros(tracker):
    stats = await tracker.get_stats('neverrecorded.com')
...
async def test_reset_specific_domain(tracker):
    await tracker.record_url('https://example.com/a', used_llm=True)
    await tracker.record_url('https://other.com/b', used_llm=True)
    await tracker.reset('example.com')
    assert await tracker.get_url_count('example.com') == 0
    assert await tracker.get_url_count('other.com') == 1
...
async def test_print_stats_empty(tracker, capsys):
    await tracker.print_stats()
    captured = capsys.readouterr()
...
async def test_get_llm_calls_url_uses_extract_domain(tracker):
    ...
    assert await tracker.get_llm_calls('https://www.example.com/b') == 1
...
async def test_get_url_count_url_uses_extract_domain(tracker):
    ...
    assert await tracker.get_url_count('https://www.example.com/b') == 1
...
async def test_reset_domain_removes_only_that_domain(tracker):
    await tracker.record_url('https://a.com/x', used_llm=True)
    await tracker.record_url('https://b.com/y', used_llm=True)
    await tracker.reset('a.com')
    stats = await tracker.get_all_stats()
    assert 'a.com' not in stats
    assert 'b.com' in stats


async def test_reset_nonexistent_domain_no_error(tracker, capsys):
    await tracker.reset('ghost.com')
    captured = capsys.readouterr()
...
async def test_load_data_returns_empty_dict_for_invalid_json(tmp_path):
    ...
    tracker = LLMTracker(tracking_file=str(tracking_file))
    data = await tracker._load_data()
    assert data == {}


async def test_get_stats_returns_zeros_for_unknown_domain(tracker):
    stats = await tracker.get_stats('unknown.xyz')
    assert stats.llm_calls == 0
    assert stats.url_count == 0
...
async def test_save_data_uses_ensure_ascii_false(tracker):
    ...
    await tracker._save_data(data)
    loaded = await tracker._load_data()
...
async def test_get_llm_calls_uses_extract_domain_for_url_with_scheme(tracker):
    ...
    count = await tracker.get_llm_calls('https://www.test.com/other')
    assert count == 1
...
async def test_get_llm_calls_uses_domain_directly_without_scheme(tracker):
    ...
    count = await tracker.get_llm_calls('test.com')
    assert count == 1
...
async def test_reset_specific_domain_saves_data(tracker):
    ...
    assert await fresh.get_url_count('a.com') == 0
    assert await fresh.get_url_count('b.com') == 1
...
async def test_reset_all_saves_empty_data(tracker):
    ...
    await tracker.record_url('https://a.com/x', used_llm=True)
    await tracker.reset()
    assert await tracker._load_data() == {}
