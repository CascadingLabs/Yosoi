...
- Pattern: a `TYPE_CHECKING` block with `from x import Y as Y` (for mypy/IDEs) + a `_LAZY = {name: "dotted.module"}` map + `__getattr__, __dir__ = lazy_exports(__name__, globals(), _LAZY)`. The top-level `yosoi/__init__.py` is also mirrored by `yosoi/__init__.pyi` — keep them in lockstep.
