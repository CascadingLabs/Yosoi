"""Benchmark: vc (Rust/chromiumoxide pool) vs zendriver on a WAF-protected page.
...
Run:
  uv run python benchmarks/bench_vc_vs_zd.py
  uv run python benchmarks/bench_vc_vs_zd.py --runs 3 --parallel 3
  uv run python benchmarks/bench_vc_vs_zd.py --vc-only --mode balanced
"""
...
async def bench_vc(url: str, runs: int, parallel: int, mode: str, *, headless: bool = False) -> Result:
    ...
    label = f'vc {mode} ({vp.width}x{vp.height}, {cfg.effective_browsers}b×{cfg.effective_tabs_per_browser}t)'
    r = Result(label)
    gc.collect()
    ...
    async def _vc_fetch(pool_ref: object) -> tuple[int, float, bool, str]:
    ...
    try:
        for i in range(runs):
            length, elapsed, blocked, event = await _vc_fetch(pool)
        ...
        if parallel > 1:
            ...
            results = await asyncio.gather(*[_vc_fetch(pool) for _ in range(parallel)])
...
def _print_comparison(vc_r: Result, zd_r: Result) -> None:
    """Print head-to-head comparison between an vc result and zendriver."""
    print('\n  vs zendriver:')

    vc_avg = statistics.mean(vc_r.single_fetches) if vc_r.single_fetches else 0
    zd_avg = statistics.mean(zd_r.single_fetches) if zd_r.single_fetches else 0
    if vc_avg and zd_avg:
        faster = 'vc' if vc_avg < zd_avg else 'zd'
        ratio = max(vc_avg, zd_avg) / min(vc_avg, zd_avg)
        print(f'    Single fetch:    {faster} is {ratio:.1f}x faster')

    if vc_r.parallel_time and zd_r.parallel_time:
        faster = 'vc' if vc_r.parallel_time < zd_r.parallel_time else 'zd'
        ratio = max(vc_r.parallel_time, zd_r.parallel_time) / min(vc_r.parallel_time, zd_r.parallel_time)
        print(f'    Parallel fetch:  {faster} is {ratio:.1f}x faster')

    faster = 'vc' if vc_r.cold_start < zd_r.cold_start else 'zd'
    ratio = max(vc_r.cold_start, zd_r.cold_start) / min(vc_r.cold_start, zd_r.cold_start)
    print(f'    Cold start:      {faster} is {ratio:.1f}x faster')

    if vc_r.chrome_rss_peak and zd_r.chrome_rss_peak:
        lighter = 'vc' if vc_r.chrome_rss_peak < zd_r.chrome_rss_peak else 'zd'
        diff = abs(vc_r.chrome_rss_peak - zd_r.chrome_rss_peak)
        print(f'    Chrome RSS:      {lighter} uses {diff:.0f} MB less')

    if vc_r.mem_efficiency and zd_r.mem_efficiency:
        better = 'vc' if vc_r.mem_efficiency < zd_r.mem_efficiency else 'zd'
        ratio = max(vc_r.mem_efficiency, zd_r.mem_efficiency) / min(vc_r.mem_efficiency, zd_r.mem_efficiency)
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='Benchmark vc vs zendriver')
    ...
    parser.add_argument('--vc-only', action='store_true', help='Only run vc benchmarks')
    parser.add_argument('--zd-only', action='store_true', help='Only run zendriver benchmark')
    parser.add_argument(
        '--mode',
        choices=['full', 'balanced', 'lite', 'all'],
        default='all',
        help='vc performance mode (default: all three)',
    )
    ...
    vc_modes = ['full', 'balanced', 'lite'] if args.mode == 'all' else [args.mode]
