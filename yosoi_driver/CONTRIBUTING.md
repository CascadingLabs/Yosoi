...
yosoi_driver/
├── crates/
│   ├── core/              # yosoi_driver_core — pure Rust CDP wrapper
│   │   ├── src/           # Library source
│   │   └── tests/         # Integration tests (require Chrome)
│   └── pyo3_bindings/     # yosoi_driver — PyO3 extension module
...
3. **Type stub** (`yosoi_driver.pyi`): Add the async signature
...
- `cargo test -p yosoi_driver_core -- --test-threads=1` — Rust integration tests

Renovate manages Cargo dependency updates in the `yosoi-driver-rust-deps` group.
