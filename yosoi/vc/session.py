...
class SessionConfig(BaseModel, frozen=True):
    ...
    Example::
        ...
        async with vc.session(headless=False, stealth=True) as browser:
...
def session(config: SessionConfig | None = None, **kwargs: Any) -> BrowserSession:
    ...
    Use as an async context manager::
        ...
        async with vc.session(headless=False) as browser:
            page = await browser.new_page("https://example.com")
            html = await page.content()
    ...
    Returns:
        A :class:`~void_crawl.BrowserSession` ready for ``async with``.

    Raises:
        ImportError: If ``void_crawl`` is not installed.
