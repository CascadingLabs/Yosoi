...
from yosoi.vc._import import require_driver
...
class PoolConfig(BaseModel, frozen=True):
    ...
    fields for full control::
        ...
        async with vc.pool(mode='balanced') as p:
            ...
        ...
        async with vc.pool(
            ...
            viewport=vc.Viewport(width=1024, height=768),  # custom viewport
        ) as p:
            ...
    """
...
async def create_pool(config: PoolConfig | None = None, **kwargs: Any) -> BrowserPool:
    ...
    Returns:
        A :class:`~void_crawl.BrowserPool` (not yet entered).
...
async def pool(config: PoolConfig | None = None, **kwargs: Any) -> AsyncIterator[BrowserPool]:
    ...
    Example::
        ...
        async with vc.pool(mode='full') as p:       # server
            ...
        async with vc.pool(mode='balanced') as p:    # desktop
            ...
        async with vc.pool(mode='lite') as p:        # embedded/CI
            ...

        # Fine-grained
        async with vc.pool(
            mode='full',
            viewport=vc.Viewport(width=1440, height=900),
        ) as p:
    ...
    Yields:
        A :class:`~void_crawl.BrowserPool` that is cleaned up on exit.
