...
```bash
uvx yosoi research init "research topic"
```
...
```bash
uvx yosoi research init "research topic" --llm-budget-usd 40 --api-budget-usd 60
```
...
```bash
uvx yosoi crawl "https://example.com" --limit 20 --json --policy PACKET/policy.yaml > PACKET/sources/crawl-example.json
```
