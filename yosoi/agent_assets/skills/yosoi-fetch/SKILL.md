...
```bash
uvx yosoi fetch "https://example.com/page" --chars 12000 --json
```
...
```bash
uvx yosoi fetch "https://example.com/page" --chars 12000 --page 2 --json
```
...
```bash
uvx yosoi fetch URL --view markdown --chars 12000
```
...
```bash
uvx yosoi fetch URL --view clean-html --chars 20000 --json
```
...
```bash
uvx yosoi fetch URL --view links --json
```
...
```bash
uvx yosoi fetch URL --view metadata --include headers,network,fingerprint,links --json
```
...
```bash
uvx yosoi fetch URL --view ax --json
```
...
```bash
uvx yosoi fetch URL --view bundle --output .yosoi/fetches/example-page --json
```
...
```bash
uvx yosoi fetch URL --fetcher simple --view raw-html
uvx yosoi fetch URL --fetcher headless --view rendered-html
uvx yosoi fetch URL --fetcher auto --view text
```
...
```bash
uvx yosoi fetch URL --chars 8000 --page 1 --json
uvx yosoi fetch URL --chars 8000 --page 2 --json
```
...
```bash
uvx yosoi fetch URL --view bundle --output .yosoi/fetches/name --json
```
...
```bash
uvx yosoi fetch URL --contract @NewsArticle --view metadata --json
uvx yosoi fetch URL --contract @Product --contract @NewsArticle --view metadata --json
```
...
```bash
uvx yosoi fetch URL --chars 12000 --json
```
...
```bash
uvx yosoi fetch URL --view raw-html --chars 20000 --json > /tmp/static.json
uvx yosoi fetch URL --view rendered-html --chars 20000 --json > /tmp/rendered.json
```
...
```bash
uvx yosoi fetch URL --contract @NewsArticle --view metadata --include fingerprint --json
```
...
uvx yosoi fetch URL --view clean-html --chars 20000 --json
uvx yosoi fetch URL --view metadata --include headers,network,fingerprint,links --json
uvx yosoi fetch URL --contract @Contract --view metadata --json
```
...
```bash
uvx yosoi fetch URL --policy .yosoi/policy.yaml --json
```
