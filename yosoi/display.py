...
def show(
    ...
    *,
    ...
    """Render scraped data in a terminal-friendly form.

    Args:
        value: Data to display. Scrape-shaped lists and mappings render as Rich
            tables in ``auto`` mode.
        format: Display format: ``auto``, ``table``, ``plain``, or ``json``.
        title: Optional heading printed before the rendered value.
        console: Rich console to write to. Defaults to a shared console.
