...
MIN_AX_RENDER_TIER = 2


@dataclass(frozen=True)
class AxTarget:
    """Replayable accessibility target for a named AX node."""
...
def ax_available_for_render_tier(render_tier: int) -> bool:
    """Return whether a capture tier has a browser-computed AX tree."""
    return render_tier >= MIN_AX_RENDER_TIER


def snapshot_for_render_tier(nodes: list[dict[str, Any]], render_tier: int) -> AxSnapshot | None:
    """Build an AX snapshot only for browser-rendered L2+ captures."""
    if not ax_available_for_render_tier(render_tier):
        return None
